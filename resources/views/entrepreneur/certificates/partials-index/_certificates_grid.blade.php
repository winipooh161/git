<div id="certificates-container" class="grid-container">
    ...
    <div class="row row-cols-3 row-cols-sm-3 row-cols-md-3 row-cols-lg-4 row-cols-xl-5 card-grid certificate-row grid-visual">
        ...
        @foreach ($certificates as $certificate)
            <div class="col certificate-item h-100 grid-cell" data-recipient-name="{{ $certificate->recipient_name }}"
                data-recipient-phone="{{ $certificate->recipient_phone }}"
                data-certificate-number="{{ $certificate->certificate_number }}">
                <div class="card border-0 shadow-sm h-100 certificate-card {{ $certificate->status === 'series' ? 'series-card' : '' }}"
                    data-certificate-id="{{ $certificate->id }}"
                    data-public-url="{{ route('certificates.public', $certificate->uuid) }}"
                    data-certificate-number="{{ $certificate->certificate_number }}"
                    data-is-batch-parent="{{ $certificate->is_batch_parent ? 'true' : 'false' }}"
                    data-batch-size="{{ $certificate->batch_size }}"
                    data-activated-copies="{{ $certificate->activated_copies_count }}">
                    <a href="{{ route('certificates.public', $certificate->uuid) }}" class="card-link"
                        ...
                        <div class="certificate-actions" onclick="event.stopPropagation();">
                            ...
                            <button type="button" 
                               ...
                               data-certificate-uuid="{{ $certificate->uuid }}"
                               data-certificate-number="{{ $certificate->certificate_number }}"
