...
<head>
    ...
    <title>{{ $certificate->template->name }} - {{ config('app.name') }}</title>
    
    <!-- Мета-теги для SEO и социальных сетей -->
    <meta name="description" content="Подарочный сертификат от {{ $data['company_name'] ?? config('app.name') }}">
    <meta property="og:title" content="{{ $certificate->template->name }} - {{ config('app.name') }}">
    <meta property="og:description" content="Подарочный сертификат от {{ $data['company_name'] ?? config('app.name') }}">
...
<body class="bg-gray-100">
    ...
    <div class="main-container" id="mainContainer">
        ...
        @include('certificates.partials.certificate-section', ['certificateData' => $certificateData])
        
        <!-- Информация о серии сертификатов -->
        @if(isset($certificate) && ($certificate->status === 'series' || $certificate->is_batch_parent))
            <div class="series-info-container">
                <div class="series-info-box">
                    ...
                    <p>
                        Этот сертификат выпущен как серия из <strong>{{ $certificate->batch_size }}</strong> экземпляров.
                        @if($certificate->activated_copies_count < $certificate->batch_size)
                            <span class="text-success">
                                Доступно для активации: <strong>{{ $certificate->batch_size - $certificate->activated_copies_count }}</strong> экз.
                            </span>
                        @else
                    ...
                    <div class="progress mt-3" style="height: 10px;">
                        <div class="progress-bar bg-primary" role="progressbar" 
                             style="width: {{ ($certificate->activated_copies_count / $certificate->batch_size) * 100 }}%;" 
                             aria-valuenow="{{ $certificate->activated_copies_count }}" 
                             aria-valuemin="0" 
                             aria-valuemax="{{ $certificate->batch_size }}"></div>
                    </div>
                    <div class="d-flex justify-content-between small text-muted mt-1">
                        <span>Активировано: {{ $certificate->activated_copies_count }}</span>
                        <span>Всего: {{ $certificate->batch_size }}</span>
                    </div>
                </div>
            </div>
        @endif
    ...
    <script>
        window.certificateData = @json($certificateData);
    </script>
    ...
    @elseif($certificate->status === 'series' && $certificate->activated_copies_count >= $certificate->batch_size)
    ...
    <div id="authModal" class="auth-modal-overlay">
        <div class="auth-modal">
            ...
            <div id="loginForm" class="auth-form">
                <form action="{{ route('login') }}" method="POST" id="certificateLoginForm">
                    @csrf
            ...
            <div id="registerForm" class="auth-form" style="display: none;">
                <form action="{{ route('register') }}" method="POST" id="certificateRegisterForm">
                    @csrf
                    <input type="hidden" name="certificate_uuid" value="{{ $certificate->uuid }}">
        ...
        document.addEventListener('DOMContentLoaded', function() {
                ...
                getCertificateBtn.addEventListener('click', function() {
                    ...
                    .then(data => {
                        if (data.authenticated) {
                            ...
                            const claimUrl = '{{ route("certificates.claim", $certificate->uuid) }}';
            ...
            if (certificateLoginForm) {
                certificateLoginForm.addEventListener('submit', function(e) {
                    ...
                    redirectInput.value = '{{ route("certificates.claim", $certificate->uuid) }}';
